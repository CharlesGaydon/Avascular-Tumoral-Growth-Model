# Final version of the Tumoral Growth Model (with genetic network)
...
class Site:
    def __init__(self, i, j, cell_id, cell_type):
        ...
        if self.cell_type == NON_TUMOR:
            self.concentration = [0.28, 5.5, 0.0, 1.0, 0.0] #o2, glucose, waste, gF, ihF
        else:
...
class Cell:
    def __init__(self, sites):
        self.cell_type = PROLIFERATING
        self.sites = sites
        self.genes = {TGFb : [[0,1],factor], 
                SCF : [[0,1],factor], 
                GSK3b : [[0,1],factor], 
                                E2F : [[0,1],[Rb]], 
                                Rb : [[0,1],[CyCD,CyCE]],
                                CyCD : [[1,0],[GSK3b,p15,p27]],
                                CyCE : [[1,0],[p27,p21]],
                                p15 : [[1,0],[SMAD]],
                                p27 : [[1,0],[SMAD,SCF]],
                                p21 : [[1,0],[SCF]],
    ...
    def update_state(self, grid, radius):
        ...
        for i, j in self.sites:
            ...
            site = grid2[i][j]
            f = self.update_network(concentration)
            if f == 1 and site.cell_type == QUIESCENT:
                p = 1
                if random.uniform(0, 1) < p:
                    grid2[i][j].cell_type = PROLIFERATING
                    site.cell_type = PROLIFERATING
            elif site.cell_type == QUIESCENT and concentration[0] < 0.02 and concentration[1] < 0.06 and concentration[2] > 8:
                p = 1
                if random.uniform(0, 1) < p:
    ...
    def update_network(self, C):
		dic = self.genes
		f = 1/(1 + exp(-(alpha*(C[3]-C[4])/initgF) - theta))
		if f>threshold : 
			dic[TGFb][0][0] = 1
			dic[GSK3b][0][0] = 1
			dic[SCF][0][0] = 1
			...
			for cle, value in dic.items():
				if value[1] != factor:
					ok = True
					for ant in value[1] : 
						if diccop[ant][0][0] != diccop[ant][0][1]:
							ok = False
					value[0][0] = ok
			
		else:
			dic[TGFb][0][0] = (random.uniform(0, 1) < f/10)
			dic[GSK3b][0][0] = (random.uniform(0, 1) < f/10)
			dic[SCF][0][0] = (random.uniform(0, 1) < f/10)
            #Theoriquement on change les trois genes en amont mais ca marchait pas...
		return dic[E2F][0][0]
...
class Tumor:
    ...
    def growth(self):
		#Facteur d'aggressivite cellulaire des cellule Proliferating.
        p = 0.08
        #Parametre de vitesse de la proliferation
        vit = 0.05 #0.2
        rad = self.radius
        hits = 0
        Lu = [-1, 0, 1] 
                        ...
                        for v in Lv:
                            ...
                            if self.grid[x][y].cell_type == NON_TUMOR and ok:
                                if random.uniform(0, 1) < p:
                                    ...
                                    sites.add((x, y))
                                    ok = False #une seule reprod par tour
                                if x - self.n/2 > rad:
                                    rad = x - self.n/2
                                if y - self.n/2 > rad:
                                    rad = y - self.n/2
    ...
    def reaction_diffusion(self):
        ...
        gF = np.zeros((self.n, self.n))
        ihF = np.zeros((self.n, self.n))
        molecules = [oxy, glu, lac, gF, ihF]
        boundary_conditions = (0.28, 5.5, 0.0, 0.0, 0.0)
        for _ in xrange(n):
            ...
            dela_gF = laplacian(gF, dx)
            dela_ihF = laplacian(ihF, dx)
            deltas = [delta_oxy, delta_glu, delta_lac, dela_gF, dela_ihF]

            for i in xrange(1, self.n-1):
                for j in xrange(1, self.n-1):
                    cell_type = self.grid[i][j].cell_type
                    if cell_type == NON_TUMOR:
                        oxy[i][j], glu[i][j], lac[i][j], gF[i][j], ihF[i][j] = boundary_conditions
                    else:
                        for k in xrange(n_molecules):
...
name = 'p'+str(0.08)+'size'+str(size)+'co'+str(co)+'vit'+str(0.05)+'Tc'+str(Tc)+'.txt'
...
f = open('History/'+"NET_"+name, 'w')
f.write('\n'.join(oxy))
f.close()
print nb
infile = 'History/NET_Concentrations'+name
